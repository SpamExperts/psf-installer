...
function find_php {
    ...
    phpLocations=( "/usr/local/cpanel/3rdparty/bin/php-cgi" "/var/cpanel/3rdparty/bin/php-cgi" "/opt/psa/admin/bin/php" `which php5 2>/dev/null` `which php 2>/dev/null` "/usr/local/bin/prospamfilter_php" )
...
then
    echo "Installing ProSpamFilter for DirectAdmin.."
...
random=`< /dev/urandom tr -dc A-Za-z0-9 | head -c5 | md5sum | awk {'print $1'}`
