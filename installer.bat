...
"%plesk_dir%admin\bin\php.exe" -f "%tmp%\prospamfilter\bin\install.php"
PAUSE
exit /b

:Abort
echo Installation proces aborted.
PAUSE
exit /b
    ...
    <script language="VBScript">
        ...
        if NOT oArgs.count = 0 Then
            if oArgs(0) = "trunk" Then
                sCHECKURL = "http://download.seinternal.com/integration/?act=getversion&panel=plesk&tier=testing"
                sfilepart = "_testing.zip"
            else
                sCHECKURL = "http://download.seinternal.com/integration/?act=getversion&panel=plesk&tier=testing&branch=" & oArgs(0)
                sfilepart = "_testing_" & oArgs(0) & ".zip"
            End If
        else 
                sCHECKURL="http://download.seinternal.com/integration/?act=getversion&panel=plesk&tier=stable"
